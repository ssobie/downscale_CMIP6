...
submit_combine_job <- function(var.name,gcm,run,scenario,type) {
      ...
      comb.array[6] <- paste0("#PBS -o output=./out_files/",gcm,"-",var.name,"-",run,"-comb.out")
      comb.array[7] <- paste0("#PBS -e error=./out_files/",gcm,"-",var.name,"-",run,"-comb.err")
...
gcm.list <- 'CanESM5'
scenario <- 'ssp585'
run.list <- 'r6i1p2f1' ###c('r5i1p2f1','r6i1p2f1','r7i1p2f1','r8i1p2f1','r9i1p2f1')
var.list <- 'tasmax'

##---------------------------------------------------------------------------


comb.template <- "/storage/home/ssobie/code/repos/downscale_CMIP6/template.recombinant.files.r"
